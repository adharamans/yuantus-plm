...
| Pact verification evidence | `<sync_metasheet2_pact.sh --check --verify-provider output or CI URL>` |
| License type | `real vendor-signed` / `dogfood ephemeral-key` |
| Tenant | `<tenant id>` |
| Org | `<org id or n/a>` |

## 2. License Import And Status
...
## 3. Capability Manifest And BOM Context
