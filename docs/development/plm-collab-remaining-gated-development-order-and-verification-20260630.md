...
| Date-obsolete revert | **RATIFIED + IMPLEMENTED 2026-07-01 → DP1 (i)/(ii)** reopen + un-acknowledge review-flag revert, audited by an append-only correction event (iv), admin-gated, idempotent; **(iii) undo child-obsolete promotion DEFERRED** to a separate ratification (reverses real Item lifecycle; superuser-only when done). Built + verified (see `plm-collab-date-obsolete-revert-dp1-impl-verification-20260701.md`). Owner ratifies whether revert means reopen impact only, undo acknowledge only, undo child obsolete promotion, or create a superseding correction event. |
