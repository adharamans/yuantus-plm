...
- LLM/NL 只留 Provider 接缝，默认 `AI_PROVIDER=none`。
...
- 配置基类：`config/settings.py` 的 `class Settings(BaseSettings)`，`model_config` 含 `extra="ignore"`（:13）—— 未声明的 env 会被静默吞掉。
...
- 在 `src/yuantus/config/settings.py` **显式声明**（否则 `extra="ignore"` 静默吞掉）：
...
- env 必须在 `Settings` 模型声明，否则 `extra="ignore"` 静默丢弃（命中 `AI_PROVIDER`）。
